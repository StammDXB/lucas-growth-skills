...
Add to copy-editing § "Sweep 1: Clarity / What to check" a new bullet:

```
**System messages and error strings (when present in the copy under review):**
Apply the What/Why/How formula (Paul Bakaus, paul-bakaus-008): each error must answer
(1) What happened? (2) Why? (3) How to fix it?
Separately, check for blame framing in Sweep 2: 'You entered an invalid date' should be
rewritten as 'Please enter a date in MM/DD/YYYY format'. Blame framing violates the "you rule"
(copy speaks to the reader, not against them).
```
